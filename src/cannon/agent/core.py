...
from cannon.validation.validator import validate_findings
...
def _build_dynamic_context(deps: CannonDeps) -> str:
...
def _build_child_brief(parent_deps: CannonDeps, child_target: str) -> str:
...
async def _spawn_child(
    parent_deps: CannonDeps, target: str, prompt: str
) -> dict[str, Any]:
    ...
    async with sem:
        ...
        child_log.info(
            "[cannon.phase]Child agent starting[/] for [cannon.target]%s[/]", target
        )
            ...
            async for node in agent_run:
                if Agent.is_model_request_node(node):
                    ...
                    if deps._unproductive_streak >= _UNPRODUCTIVE_THRESHOLD * 2:
                        log.info(
                            "[cannon.phase]Early termination[/]: %d unproductive iterations",
...
#!/usr/bin/env python3
"""
Cannon — Finding Validation Script
====================================
...
vulnerability.  No Cannon installation required — stdlib only.
...
def main():
    ...
    print(f"\\n{{BOLD}}Cannon Finding Validator{{RESET}}")
